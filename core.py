...
def excel_table_byname(filepath, colnameindex=0, by_name=u'汉字部件拆分表'):
...
def draw_word(root, word, start_x, start_y, gezi_width, getzi_height, font_path, font_size):
    ...
    text = font.render(word.decode("utf-8"), True, (0, 0, 0))
...
def All_create(excel, out):
    ...
    for row in tables:
        ...
        try:
            ...
            # 泽华负责把部件,循环调用draw_gezi_and_word方法，x y坐标要设计好,大小每个50像素，每行4个，card_width为300
            ...
            # draw_bigRect(root)
            continue
        save_to_png(root, out_path + filename+row[1] + ".png")
...
def One_create(excel, out, word):
    ...
    draw_pingyin_number(root, result_row[1], filename, "C:\Windows\Fonts\simsun.ttc")
    # 画格子+画字
    try:
        draw_gezi_and_word(root, 2, big_gezi_x, big_gezi_y, big_gezi_size, big_gezi_size, result_row[1],
                           "C:\Windows\Fonts\simsun.ttc", 140, (0, 0, 0))
        # 泽华负责把部件,循环调用draw_gezi_and_word方法，x y坐标要设计好,大小每个50像素，每行4个，card_width为300
        draw_bushou(root, "C:\Windows\Fonts\simsun.ttc", result_row)
        draw_bigRect(root)
    except Exception, e:
        draw_gezi_and_word(root, 2, big_gezi_x, big_gezi_y, big_gezi_size, big_gezi_size, result_row[1],
                           "C:\Windows\Fonts\simsun.ttc", 140, (0, 0, 0))
        ...
        draw_bushou(root, "C:\Windows\Fonts\simsun.ttc", result_row)
        draw_bigRect(root)
    save_to_png(root, out_path + filename+result_row[1] + ".png")
...
class MainWindow(wx.Frame):
    ...
    def OnBeginClick(self, event):
        ...
        if (self.m_checkBox1.GetValue() == False):
            result_msg=All_create(self.excelfile_et.GetPath(), self.outpath_et.GetPath() + "\\")

        else:
            # 生成单个汉字
            result_msg=One_create(self.excelfile_et.GetPath(), self.outpath_et.GetPath() + "\\", self.word_et.GetValue())


        dlg = wx.MessageDialog(self, result_msg, "message", wx.OK)  # 语法是(self, 内容, 标题, ID)
        dlg.ShowModal()  # 显示对话框
        dlg.Destroy()  # 当结束之后关闭对话框
